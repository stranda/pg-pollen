...
species_names <- readRDS(here::here('data', 'taxa_4.1.RDS'))
...
for (species_to_plot in unique(dat_pi_mean$species)) {
   print(species_to_plot) 
        ###first make rasterbricks of the mean and sd at resolution fit from model
        spdf_mn <- dat_pi_mean %>%
            filter(species == species_to_plot)
        spdf_sd <- dat_pi_sd %>%
            filter(species == species_to_plot)
        proj <- '+proj=aea +lat_0=37.5 +lon_0=-96 +lat_1=29.5 +lat_2=45.5 +x_0=0 +y_0=0 +ellps=GRS80 +units=m +no_defs'
        rbrick_mn <- brick(lapply(sort(unique(spdf_mn$time)),function(tim)
        {
            rasterFromXYZ(spdf_mn[spdf_mn$time==tim,c("x","y","pi_mean")],crs=proj)
        }))
        rbrick_sd <- brick(lapply(sort(unique(spdf_sd$time)),function(tim)
        {
            rasterFromXYZ(spdf_sd[spdf_sd$time==tim,c("x","y","pi_sd")],crs=proj)
        }))

   writeRaster(rbrick_mn,format="GTiff",overwrite=TRUE,options=c("COMPRESS=LZW"),filename=here::here("figures", "matern_overdispersed", paste0("predictions-", species_to_plot, "_meanpred.tif")),)
   writeRaster(rbrick_sd,format="GTiff",overwrite=TRUE,options=c("COMPRESS=LZW"),filename=here::here("figures", "matern_overdispersed", paste0("predictions-", species_to_plot, "_sdpred.tif")),)
                
   print("finished saving prediction rasters")

   ##now make nice plots    
        p_mean <- ggplot(spdf,aes(x = x, y = y, fill = pi_mean)) +
            geom_raster() +
            geom_polygon(data = dat_cont_shp,
                         aes(x = long, y = lat, group = group), fill = NA,
                         color = 'black', size = .2) +
        ...
        p_sd <- ggplot(spdf_sd,aes(x = x, y = y, fill = pi_sd)) +
            geom_raster() +
            geom_polygon(data = dat_cont_shp,
                         aes(x = long, y = lat, group = group), fill = NA,
                         color = 'black', size = .2) +
